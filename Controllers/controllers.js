...
const dogsController = {
  ...
  deleteDog: (req, res) => {
    fs.readFile(dataPath, "utf-8", (err, data) => {
      ...
      if (err) return res.status(500).send("Erreur de lecture du fichier");
      const dogs = JSON.parse(data);
      const updateDogs = dogs.filter(d => d.id !== req.params.id);
      if(updateDogs.length === dogs.length) return res.status(404).send("Chien non trouvé");

      fs.writeFile(dataPath, JSON.stringify(updateDogs), 'utf-8', (err) => {
        if (err) return res.status(500).send('Erreur écriture');
